function mMCM_validation(fmriprepdir, templatepath, thresholds, activation_thresh, taskname, firstlevdir)
clear;
tic;
...
for i = 1:length(subids)
    subid = char(subids(i));
    disp(subid)
    component_filename = [subid '/restmelodicsmoothmask_auto/' subid '_mMCM.nii'];
    if isfile(component_filename)
...
writetable(icaacttable,'mMCM_validation.csv','Delimiter',',');
toc;
end